...
export default function PostProcessing() {
  ...
  const [form, setForm] = useState<Omit<PostProcess, 'id'>>({
    orderId: orders[4]?.id ?? '', sandblastPressure: 0.6, sandblastMedia: 'Al2O3 120#',
    sandblastDuration: 20, polishingMethod: '电解抛光', targetRoughness: 1.6, actualRoughness: 1.2,
    qualityResult: '', operator: '', dimensions: []
  })
  const [dims, setDims] = useState<DimensionCheck[]>(pp?.dimensions ?? [])
  const [editing, setEditing] = useState(!pp)

  const data = editing ? form : (pp ?? form)

  const set = (k: string, v: any) => {
    if (editing) setForm(f => ({ ...f, [k]: v }))
    else if (pp) updatePostProcess(pp.id, { [k]: v })
  }
  ...
  const updateDim = (i: number, k: keyof DimensionCheck, v: any) => {
    ...
    setDims(next)
    if (!editing && pp) updatePostProcess(pp.id, { dimensions: next })
  }

  const addDim = () => {
    const d: DimensionCheck = { name: '', targetValue: 0, actualValue: 0, tolerance: 0.05, result: 'pass' }
    setDims([...dims, d])
    if (!editing && pp) updatePostProcess(pp.id, { dimensions: [...dims, d] })
  }
  ...
  const save = () => {
    const payload = { ...form, dimensions: dims }
    addPostProcess(payload)
    setEditing(false)
  }
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ position: 'relative' }}>
            <select value={selId} onChange={e => { setSelId(e.target.value); setEditing(false) }}
              style={{ ...inputStyle, appearance: 'none', paddingRight: 28, width: 200 }}>
          ...
          {editing && <button onClick={save} className="btn-primary" style={{ background: '#4A90D9', color: '#fff', border: 'none', borderRadius: 6, padding: '6px 16px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>保存</button>}
          {!editing && <button onClick={() => setEditing(true)} className="btn-primary" style={{ background: '#334155', color: '#e5e7eb', border: 'none', borderRadius: 6, padding: '6px 16px', cursor: 'pointer', fontSize: 13 }}>新建</button>}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 16, flex: 1, minHeight: 0 }}>
        <div className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 14, overflow: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            <div><div style={labelStyle}>压力 (MPa)</div><input type="number" step="0.1" value={data.sandblastPressure} onChange={e => set('sandblastPressure', +e.target.value)} style={inputStyle} /></div>
            <div><div style={labelStyle}>磨料类型</div>
              <select value={data.sandblastMedia} onChange={e => set('sandblastMedia', e.target.value)} style={inputStyle}>
                {mediaOptions.map(m => <option key={m} value={m}>{m}</option>)}
              </select>
            </div>
            <div><div style={labelStyle}>时长 (min)</div><input type="number" value={data.sandblastDuration} onChange={e => set('sandblastDuration', +e.target.value)} style={inputStyle} /></div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {polishMethods.map((m, i) => {
              ...
              return <button key={m} onClick={() => set('polishingMethod', m)} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: 12, background: active ? 'rgba(74,144,217,0.15)' : '#0f172a', border: `1px solid ${active ? '#4A90D9' : '#334155'}`, borderRadius: 8, cursor: 'pointer', color: active ? '#4A90D9' : '#94a3b8', transition: 'all 0.15s' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
              <div><div style={labelStyle}>目标值 Ra</div><input type="number" step="0.1" value={data.targetRoughness} onChange={e => set('targetRoughness', +e.target.value)} style={inputStyle} /></div>
              <div><div style={labelStyle}>实测值 Ra</div><input type="number" step="0.1" value={data.actualRoughness} onChange={e => set('actualRoughness', +e.target.value)} style={inputStyle} /></div>
            </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #334155' }}>
                {['尺寸名称', '目标值', '实测值', '公差', '结果'].map(h => <th key={h} style={{ padding: '8px 6px', textAlign: 'left', color: '#64748b', fontWeight: 500, fontSize: 12 }}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {dims.map((d, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #1e293b' }}>
                  <td style={{ padding: '6px' }}><input value={d.name} onChange={e => updateDim(i, 'name', e.target.value)} style={{ ...inputStyle, width: 90 }} placeholder="名称" /></td>
              ...
              {dims.length === 0 && <tr><td colSpan={5} style={{ textAlign: 'center', padding: 30, color: '#475569' }}>暂无尺寸检测数据</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
